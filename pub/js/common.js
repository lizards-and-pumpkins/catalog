define(['jquery', 'lib/cookie', 'search_autosuggestion'], function(jQuery, cookie) {
...
/**
 * Swiping blocks handling
 */
function toggleSwipingArrows(outerContainer, innerContainer)
{
    jQuery.each(jQuery(outerContainer), function() {
        var width = jQuery(this).width();
        if (width) {
            var prevArr = createArrow('swipe-prev', jQuery(this), 'left'),
                nextArr = createArrow('swipe-next', jQuery(this), 'right'),
                scrollPosition = jQuery(this).scrollLeft();

            /* Bind scrolling */
            jQuery(this).unbind('scroll').scroll(function() {
                toggleSwipingArrows(jQuery(this), 'ul');
            });

            /* Handle arrows visibility depending on scrolling position */
            prevArr.css('opacity', scrollPosition > 0 ? 1 : 0);
            nextArr.css('opacity', jQuery(this).find(innerContainer).outerWidth(true) - width > scrollPosition ? 1 : 0);
        }
    });
}
...
{
    var arrow = outerContainer.siblings('.' + className);

    if (!arrow.length) {
        arrow = jQuery('<a/>', {
            'click': function() { swipeHorizontal(outerContainer, direction) },
            'class': className
        });
        outerContainer.after(arrow);
    }
...
{
    if (direction == 'left') {
        container.animate({ scrollLeft: (container.scrollLeft() - 150) + 'px' }, 'fast');
    } else {
        container.animate({ scrollLeft: (container.scrollLeft() + 150) + 'px' }, 'fast');
    }
}
